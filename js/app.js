...
function setupAuthForms() {
    ...
    registerForm.addEventListener('submit', (e) => {
        ...
        if (role !== 'admin' && role !== 'user') {
            registerError.textContent = 'Rol inválido. Debe ser "admin" o "user".';
            return;
        }
...
function setupEventListeners() {
    ...
    document.getElementById('add-tournament-btn').addEventListener('click', () => {
        document.getElementById('tournamentModalLabel').textContent = 'Crear Torneo';
    ...
    document.getElementById('add-team-btn').addEventListener('click', () => {
        document.getElementById('teamModalLabel').textContent = 'Crear Equipo';
        clearModalForm('team-form');
        populateSelect('team-tournament', getItem('tournaments') || [], 'id', 'name', 'Seleccionar Torneo');
    });
    ...
    document.getElementById('add-player-btn').addEventListener('click', () => {
        document.getElementById('playerModalLabel').textContent = 'Crear Jugador';
        clearModalForm('player-form');
        populateSelect('player-team', getItem('teams') || [], 'id', 'name', 'Seleccionar Equipo');
    });
    ...
    document.getElementById('add-match-btn').addEventListener('click', () => {
        document.getElementById('matchModalLabel').textContent = 'Crear Partido';
        clearModalForm('match-form');
        populateSelect('match-tournament', getItem('tournaments') || [], 'id', 'name', 'Seleccionar Torneo');
        // Team selects will be populated on tournament change
        document.getElementById('match-team-a').innerHTML = '<option value="">Seleccione Torneo Primero</option>';
        document.getElementById('match-team-b').innerHTML = '<option value="">Seleccione Torneo Primero</option>';
    });
    ...
    document.getElementById('match-tournament').addEventListener('change', (e) => {
        ...
        populateSelect('match-team-a', tournamentTeams, 'id', 'name', 'Equipo Local');
        populateSelect('match-team-b', tournamentTeams, 'id', 'name', 'Equipo Visitante');
    ...
    document.getElementById('add-stat-field-team-a').addEventListener('click', () => {
        ...
        else alert('Seleccione un equipo local primero.');
    });
    document.getElementById('add-stat-field-team-b').addEventListener('click', () => {
        ...
        else alert('Seleccione un equipo visitante primero.');
    });
...
function handleTournamentTableActions(e) {
    ...
    if (target.classList.contains('edit-tournament-btn')) {
        ...
        if (tournament) {
            document.getElementById('tournamentModalLabel').textContent = 'Editar Torneo';
        ...
        if (confirm('¿Estás seguro de eliminar este torneo? Esto también eliminará equipos y partidos asociados.')) {
            let tournaments = getItem('tournaments') || [];
...
function handleTeamFormSubmit(e) {
    ...
    if (!team.name || !team.tournamentId) {
        alert("Nombre y torneo son obligatorios.");
        return;
    }
    let teams = getItem('teams') || [];
    if (id) {
...
function handleTeamTableActions(e) {
    ...
    if (target.classList.contains('edit-team-btn')) {
        ...
        if (team) {
            document.getElementById('teamModalLabel').textContent = 'Editar Equipo';
            populateSelect('team-tournament', getItem('tournaments') || [], 'id', 'name', 'Seleccionar Torneo');
        ...
        if (confirm('¿Estás seguro de eliminar este equipo? Esto también eliminará jugadores asociados.')) {
            let teams = getItem('teams') || [];
...
function handlePlayerFormSubmit(e) {
    ...
    const player = {
     ...
     if (!player.name || !player.teamId || !player.position || isNaN(player.number)) {
        alert("Todos los campos son obligatorios y el número debe ser válido.");
        return;
    }
    let players = getItem('players') || [];
    if (id) {
...
function handleMatchFormSubmit(e) {
    ...
    if (!match.tournamentId || !match.teamAId || !match.teamBId || !match.date) {
        alert("Torneo, equipos y fecha son obligatorios.");
        return;
    }
...
function populateTournamentSelects() {
    const tournaments = getItem('tournaments') || [];
    populateSelect('leaderboard-tournament-select', tournaments, 'id', 'name', 'Seleccionar Torneo');
    populateSelect('stats-tournament-select', tournaments, 'id', 'name', 'Seleccionar Torneo');
}
...
function displayLeaderboard(tournamentId) {
    ...
    teams.forEach(team => {
        teamStats[team.id] = {
            name: team.name,
            pj: 0, g: 0, e: 0, p: 0, // Partidos Jugados, Ganados, Empatados, Perdidos
            gf: 0, gc: 0, dg: 0, pts: 0 // Goles Favor, Contra, Diferencia, Puntos
        };
    });
